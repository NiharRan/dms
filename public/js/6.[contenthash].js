...
/* harmony default export */ __webpack_exports__["default"] = ({
  ...
  methods: {
    ...
    checkStock: function checkStock(index) {
      var selectedRow = this.form.sale_details[index];
      var stock = selectedRow.product.quantity;
      var quantity = selectedRow.quantity;
      var dif = stock - quantity;

      if (dif < 0) {
        this.invalid_stock = true;
        this.form.sale_details[index].stock_alert = "".concat(stock, " - ").concat(quantity, " = ").concat(dif);
      } else {
        this.invalid_stock = false;
        this.form.sale_details[index].stock_alert = "";
      }
    },
    checkProduct: function checkProduct(obj) {
      console.log(obj);
      var result = this.form.sale_details.find(function (sale_detail) {
        return sale_detail.product.id == obj.id;
      });

      if (result) {
        this.$toast("Oops! This product is already selected!", "error");
        this.form.sale_details[this.form.sale_details.length - 1].product = null;
      }
    },
