...
class BanglaNumberToWord
{
  ...
  private function numberSelector($number)
  {
    switch ($number) {
      case $number > 9999999:
  ...
  private function numToBn($number)
  {
    return strtr($number, $this->num_to_bd);
  }
  private function numToBnDecimal($number)
  {
    return strtr($number, $this->num_to_bn_decimal);
  }

  private function crore($number)
  {
    $a = intval($number / 10000000);
    $b = intval($number % 10000000);

    $moreThenCrore = ($a > 99) ? $this->lakh($a) : $this->numToBn($a);
    return $moreThenCrore . ' ' . $this->cor . ' ' . $this->lakh($b);
  }

  private function lakh($number)
  {
    $a = intval($number / 100000);
    $b = intval($number % 100000);

    $lakh = ($a == 0) ? '' : $this->numToBn($a) . ' ' . $this->lk;
    return $lakh . ' ' . $this->thousand($b);
  }

  private function thousand($number)
  {
    $a = intval($number / 1000);
    $b = intval($number % 1000);

    $thousand = ($a == 0) ? '' : $this->numToBn($a) . ' ' . $this->thou;
    return $thousand . ' ' . $this->hundred($b);
  }

  private function hundred($number)
  {
    $a = intval($number / 100);
    $b = intval($number % 100);

    $hundred = $a === 0 ? '' : $this->numToBn($a) . ' ' . $this->hd;
    return $hundred . ' ' . $this->underHundred($b);
  }

  private function underHundred($number)
  {
